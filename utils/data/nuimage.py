    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if self.prompt_version == 'v1':
                ...
                index = index[:self.max_boxes] # 9+3*22+2=77
                ...
                for each in index:
                    ...
                    if label in ['truck', 'bicycle', 'motorcycle']:
                        ...
                        bbox_mask[coord[1]: coord[3], coord[0]: coord[2]]\
                            = self.foreground_loss_weight * 1 / torch.pow(torch.tensor((coord[3] - coord[1] + 1) * (coord[2] - coord[0] + 1)), 0.2) if self.foreground_loss_mode == 'constant' else \
                                                                        1 / torch.pow(torch.tensor((coord[3] - coord[1] + 1) * (coord[2] - coord[0] + 1)), self.foreground_loss_weight)
            ...
            boxes_pad = torch.zeros((self.max_boxes, 4), dtype=bboxes.dtype, device=bboxes.device)
            labels_pad = torch.zeros((self.max_boxes, 1), dtype=bboxes.dtype, device=bboxes.device)
            mask = torch.zeros((self.max_boxes,), dtype=torch.bool, device=bboxes.device)

            if num_bboxes > self.max_boxes:
                boxes_pad[:self.max_boxes] = bboxes[:self.max_boxes]
                labels_pad[:self.max_boxes] = labels[:self.max_boxes].unsqueeze(1)
                mask[:self.max_boxes] = True
            else:
                boxes_pad[:num_bboxes] = bboxes
                labels_pad[:num_bboxes] = labels.unsqueeze(1)
                mask[:num_bboxes] = True

            boxes_pad = torch.cat((boxes_pad, labels_pad), dim=1)
            
            example = {}
            example["pixel_values"] = data['img'].data
            example["text"] = text
            example["bbox"] = boxes_pad
            example["bbox_valid"] = mask
